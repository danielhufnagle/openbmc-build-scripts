...
# This version uses RHEL 10 UBI (Universal Base Image) as the base container.
#
###############################################################################
#
# Script Variables:
...
#                     Default: "openbmc/rhel10-ubi:${imgtag}-${target}"
#  img_tag            The base docker image distro tag:
#                     rhel10-ubi: latest|10.2
#                     Default: "latest"
...
# Configure Docker build for RHEL 10 UBI
...
Dockerfile=$(
  cat <<EOF
FROM ${docker_reg}/ubi10/ubi:${img_tag}

${PROXY}

# Enable additional repositories for RHEL 10 UBI
...
img_name=${img_name:-openbmc/rhel10-ubi:${img_tag}-${target}}

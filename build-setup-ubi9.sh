...
# This version uses RHEL 9 UBI (Universal Base Image) as the base container.
#
###############################################################################
#
# Script Variables:
...
#                     instead of our default (registry.access.redhat.com)
...
#                     Default: "openbmc/rhel9-ubi:${imgtag}-${target}"
#  img_tag            The base docker image distro tag:
#                     rhel9-ubi: latest|9.5
#                     Default: "latest"
...
docker_reg=${DOCKER_REG:-"registry.access.redhat.com"}
...
# Configure Docker build for RHEL 9 UBI
...
Dockerfile=$(
  cat <<EOF
FROM ${docker_reg}/ubi9/ubi:${img_tag}

${PROXY}

# Enable additional repositories for RHEL 9 UBI
...
img_name=${img_name:-openbmc/rhel9-ubi:${img_tag}-${target}}
